...
//        backButton()
        ...
        edtPhoneNumber.filters =
            arrayOf<InputFilter>(filterPhoneNo(edtPhoneNumber), InputFilter.LengthFilter(13))
...
//                Toast.makeText(this,"Payment Canceled",Toast.LENGTH_LONG).show()
    ...
    private fun checkEmptyData() {
        if (!TextUtils.isEmpty(pendingUCDData.buyer?.firstName)) {
            isFirstName = true
        }
        if (!TextUtils.isEmpty(pendingUCDData.buyer?.middleName)) {
            isMiddleName = true
        }
        if (!TextUtils.isEmpty(pendingUCDData.buyer?.lastName)) {
            isLastName = true
        }
        if (!TextUtils.isEmpty(pendingUCDData.buyer?.address1)) {
            isAddress1 = true
        }
        if (!TextUtils.isEmpty(pendingUCDData.buyer?.city)) {
            isCity = true
        }
        if (!TextUtils.isEmpty(pendingUCDData.buyer?.zipcode)) {
            isZipCode = true
        }
        if (!TextUtils.isEmpty(pendingUCDData.buyer?.phoneNumber)) {
            isBuyerMNo = true
        }
        if (!TextUtils.isEmpty(pendingUCDData.buyer?.email)) {
            isBuyerEmail = true
        }
        if (!TextUtils.isEmpty(pendingUCDData.buyer?.state)) {
            isState = true
        }
    }
                        ...
                        SpaySdk.SPAY_READY -> {
                            ...
                            if (mPaymentManager == null) {
                                mPaymentManager = PaymentManager(
                                    this@UCDDealSummaryStep3Activity,
    ...
    private val transactionListener: PaymentManager.CustomSheetTransactionInfoListener =
        object : PaymentManager.CustomSheetTransactionInfoListener {
            ...
            override fun onSuccess(
                ...
                val fragmentActivity: Activity = this@UCDDealSummaryStep3Activity
                if (fragmentActivity == null || fragmentActivity.isFinishing || fragmentActivity.isDestroyed) {
                    return
                }
                val mPaymentResultDialog = PaymentResultDialog(this@UCDDealSummaryStep3Activity)
